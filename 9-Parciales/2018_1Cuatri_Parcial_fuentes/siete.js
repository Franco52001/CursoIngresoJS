function mostrar() {
    ...
    var contadorvaron = 0;
        ...
        if (nota > 5 && sexo == "m" || sexo == "M") {  //este no funciona con || 
            contadorvaron++;
        }
        if(contador == 0)
        {
        ...
        if ( notamin > nota)  { //esta ***** no andaba porque soy tarado y no valide el notamin
            notamin = nota;                        //no se porque funciono afuera del for pero aca esta mejor
            sexomin = sexo;                            //contador == 1 estaba antes y no funcionaba lpm
        }
    }
